...
/*
  Visual metadata is optional and deliberately lives outside variants.js.

  Prefer passing it into buildDataIndex as rawData.visualMetadata. The global
  fallback keeps the plain-script setup tolerant while index.html is being
  updated to load data/visualMetadata.js.
*/
...
function validateAssetVisualMetadataList(assetMetadataList, index) {
  ...
  assetMetadataList.forEach(metadata => {
    ...
    validateVisualColourList(
      metadata.dominantColours,
      `${ownerLabel} dominantColours`,
      index.errors,
      true
    );
    ...
    if (
      metadata.colourCount !== undefined &&
      (!Number.isInteger(metadata.colourCount) || metadata.colourCount < 0)
    ) {
      index.errors.push(
        `${ownerLabel} colourCount must be a non-negative integer when provided.`
      );
    }

    if (
      Array.isArray(metadata.colours) &&
      Number.isInteger(metadata.colourCount) &&
      metadata.colourCount !== metadata.colours.length
    ) {
      index.warnings.push(
        `${ownerLabel} colourCount does not match colours.length.`
      );
    }
...
function validateVariantVisualMetadataList(variantMetadataList, index) {
  ...
  variantMetadataList.forEach(metadata => {
    ...
    if (
      ...
      (typeof metadata.layout !== "string" || metadata.layout.trim() === "")
    ) {
      index.errors.push(
        `${ownerLabel} layout must be a non-empty string when provided.`
      );
    }

    if (
      metadata.elements !== undefined &&
      !Array.isArray(metadata.elements)
    ) {
      index.errors.push(
        `${ownerLabel} elements must be an array when provided.`
      );
    }

    if (
      metadata.hasText !== undefined &&
      typeof metadata.hasText !== "boolean"
    ) {
      index.errors.push(
        `${ownerLabel} hasText must be a boolean when provided.`
      );
    }
...
function validateOptionalVisualMetadataBooleans(
  ...
  ["generated", "reviewed", "needsReview"].forEach(fieldName => {
    if (
...
function validateDynamicCollectionRuleNode(
  ...
  validateVisualColourRuleFields(
    ruleSet,
    collection,
    index,
    rulePath
...
function validateVisualColourRuleFields(
  ruleSet,
  collection,
  index,
  rulePath
  ...
  if (Object.prototype.hasOwnProperty.call(ruleSet, "dominantColour")) {
    validateVisualColourRuleValue(
      ...
      collection,
      index,
      `${rulePath}.dominantColour`
    );
  }

  if (Object.prototype.hasOwnProperty.call(ruleSet, "hasColours")) {
    if (!Array.isArray(ruleSet.hasColours) || ruleSet.hasColours.length === 0) {
      index.errors.push(
        `Dynamic collection ${collection.id} must use a non-empty array for ${rulePath}.hasColours.`
      );
    } else {
      const seenColours = new Set();

      ruleSet.hasColours.forEach((colour, colourIndex) => {
        const normalisedColour = validateVisualColourRuleValue(
          colour,
          collection,
          index,
          `${rulePath}.hasColours[${colourIndex}]`
        );
        ...
        if (seenColours.has(normalisedColour)) {
          index.errors.push(
            `Dynamic collection ${collection.id} contains duplicate colour in ${rulePath}.hasColours: ${colour}`
          );
        }
  ...
  if (Object.prototype.hasOwnProperty.call(ruleSet, "colourCount")) {
    if (!Number.isInteger(ruleSet.colourCount) || ruleSet.colourCount < 0) {
      index.errors.push(
        `Dynamic collection ${collection.id} must use a non-negative integer for ${rulePath}.colourCount.`
      );
    }
  }
}

function validateVisualColourRuleValue(
  value,
  collection,
  index,
  rulePath
) {
  const normalisedColour = normaliseVisualColourName(value);

  if (!normalisedColour) {
    index.errors.push(
      `Dynamic collection ${collection.id} must use a supported colour for ${rulePath}.`
    );
  }
